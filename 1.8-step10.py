#1.8 Code Challenge: Solve the Frequent Words with Mismatches and Reverse Complements Problem.
...
def ApproximatePatternCount(seq, k, d):
    ...
    for i in range(0,len(seq)-k+1):
        ...
        revNeighbors = Neighbors(revDNA(pattern),d)
        for x in revNeighbors:
            if x not in freqTable:
                freqTable[x] = 1
            else:
                freqTable[x] = freqTable[x] + 1
...
f = open("/Users/denniskenbeek/Downloads/dataset_327579_10.txt", "r")
data = f.read().split('\n')
f.close()
